...
import { isPointNode, POINT_NODE_ORIGIN } from "../type";
...
export function deriveNodeHandles(
  ...
  return nodes.map((n) => {
    ...
    return {
      ...n,
      // ポイントの position はノード中心（描画用アンカーもここで注入する）
      origin: POINT_NODE_ORIGIN,
      data: { ...n.data, handles },
    };
  });
}
...
/** ポイントの position は中心アンカー（POINT_NODE_ORIGIN）なので、そのまま中心座標 */
