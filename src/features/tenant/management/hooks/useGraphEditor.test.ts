...
describe("useGraphEditor: 同一ポイント間の複数ルート", () => {
  it("同じポイント間で　onConnect を繰り返すと独立したルートが増える", () => {
    ...
    act(() => {
      result.current.onConnect(connection("n1", "n2"));
    });
...
describe("useGraphEditor: 接続数によるハンドルの増減", () => {
  ...
  it("接続が無いときは各辺に空きハンドルが1つ", () => {
    ...
    for (const side of SIDES) {
      const slots = slotsOf(result.current.nodes, "n1", side);
  ...
  it("接続を追加するとハンドルが増え、削除すると減る（常に空き1）", () => {
    ...
    expect(usedCount(result.current.nodes, "n1", "right")).toBe(1);
    expect(freeCount(result.current.nodes, "n1", "right")).toBe(1);

    // 2本目接続: 使用済み 1 → 2 + 空き
    act(() => {
      result.current.onConnect(connection("n1", "n2"));
    });
    expect(usedCount(result.current.nodes, "n1", "right")).toBe(2);
    expect(freeCount(result.current.nodes, "n1", "right")).toBe(1); // 常に空き1

    // 1本削除: 使用済み 2 → 1 + 空き
    const removedId = result.current.edges[0].id;
    act(() => {
      result.current.selectEdge(removedId);
    });
    act(() => {
...
describe("useGraphEditor: ポイント移動によるハンドル接続位置の変化", () => {
  it("ノードを移動すると接続辺が変わる", () => {
    ...
    // 水平配置 → 右左で接続
    const before = result.current.edges.find((e) => e.id === "e1");
    ...
    const after = result.current.edges.find((e) => e.id === "e1");
